	...
	if ($nombre == "") {
		...
		if(validaCorreo($email, $conn)) {
			if ($clave1 == $clave2) {
				...
				$sql .= "'".$colonia."', '".$estado."',";
				$sql .= "'".$codpos."', '".$pais."','".$clave1."')";
				//
				if(mysqli_query($conn, $sql)) {
					header("location:registroGracias.php");
				}else {
					$errores[11]="Error al agregar a la base de datos";
				}
				}else {
					$errores[12] = "Las contraseñas no coinciden";
				}
			}else{
				$errores[13]="Ya existe el correo en la base de datos";
			}
		}
	}
